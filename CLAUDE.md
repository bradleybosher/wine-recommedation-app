...
→ filter_wine_list() coarse pass → retrieval.rank_wine_list() profile-signal ranking (top 40 lines, keyword-only — no API call;
   tiered scoring: producer +2.0, region +1.5 [synonym-expanded], grape +1.0 [synonym-expanded], style +0.5, avoided-token -2.0)
→ palate_stats.compute_palate_stats() on consumed + inventory rows → aspirational_skew summary line (no API call)
→ routes/recommend._build_tasting_note_library() → curated **TASTING NOTE LIBRARY** block from consumed rows
→ Claude (recommend) → top-N recommendations (default 3; configurable via bottle_count)
  (system prompt: taste_markers numeric block, palate_persona verbatim, tasting_note_library for evidence_quotes,
   aspirational_skew line, stretch/discovery slot for final pick when bottle_count ≥ 3;
   per-wine confidence capped at "medium" for seed-derived; color derived server-side post-validation;
   bars blended 50/50 with wine_reference.json calibrated values where appellation/grape match exists)
→ auto-saved to flights table with profile_id (cellar.db) → retrievable via GET /history scoped to active profile
→ GET /profile/insights → insights.compute_drift_suggestions() statistical analysis of flight history
  (surfaces grapes/regions Claude keeps recommending but profile omits; no LLM call; ≥ 3 flights required)
```
**Modules:** `main.py` (composition root — env, logging, middleware, router includes, legacy migration), `bootstrap.py` (.env + constants — JWT_SECRET, PROFILES_DIR, ORPHAN_PROFILE_ID), `auth.py` (password hashing + JWT utilities; FastAPI-free), `dependencies.py` (get_current_user, get_current_profile FastAPI deps), `logging_setup.py`, `middleware.py` (request log + exception handlers), `rate_limit.py`, `cellar_terms.py` (cellar character helpers), `logging_utils.py` (JSONL recommendation-event telemetry — logs to logs/recommendations.jsonl), `llm_client.py` (call_claude telemetry wrapper — logs to logs/llm_calls.jsonl), `retry_utils.py` (call_with_retry exponential-backoff helper for Anthropic calls), `wine_reviews.py` (critic-review reference dataset — seed + lookup/enrichment), `meal_parser.py` (meal description parsing — MealProfile, pairing hints), `synonyms.py` (grape/region synonym + sub-appellation expansion — no API call), `palate_stats.py` (statistical palate analysis from consumed/inventory rows — no API call; produces frequency tables, style signals, aspirational_skew, avoided_style_tokens, top_producers), `retrieval.py` (retrieval-augmented pre-filtering — tiered profile-signal ranking with synonym expansion, no API call), `insights.py` (palate drift suggestion engine — statistical flight analysis, no LLM call), `routes/{auth,profiles,inventory,profile,recommend,debug,history,insights}.py` (HTTP endpoints), `parser.py` (PDF/OCR), `models.py` (Pydantic — incl. User/Profile/TokenResponse/PalateDriftSuggestion), `recommender.py` (LLM — bar blending against wine_reference.json), `inventory.py` (cellar; profile_id-keyed), `profile.py` (taste; profile_id-keyed), `seed_profile.py` (seed-bottle onboarding; profile_id-keyed), `prompt.py` (prompt — tasting_note_library, aspirational_skew, stretch slot), `scorer.py` (scoring), `cache.py` (SQLite — users/profiles/flights + global response/parse cache + legacy migration helper), `test_fixtures.py` (canned `RecommendationResponse` fixtures for TEST_MODE), `data/wine_reference.json` (~80 canonical bars entries per appellation/grape pair, 0.0-1.0 scale) — all in `backend/`

## Recommendation Logic
- Full wine list + enriched taste profile in one prompt; reason on **style fit**, not region/varietal
- For CellarTracker uploads the palate is synthesized at upload time by `profile.synthesize_palate_from_notes()` (one Claude call on raw notes grounded by `palate_stats` statistical evidence); the per-recommendation `build_enriched_profile_text()` short-circuits when a synthesized profile is present, so the recommendation prompt sees rich multi-word descriptors, taste_markers, a `palate_persona` paragraph, and an aspirational skew line
- Tasting notes library: top-scored + bottom-scored consumed notes injected into the prompt; Claude may cite these as `evidence_quotes` (verbatim; no fabrication)
- Stretch/discovery slot: when `bottle_count >= 3`, final pick is instructed to be slightly outside the safe persona zone (`stretch=true`)
- Per-wine `bars` blended 50/50 with `data/wine_reference.json` calibrated values after validation (reference scale 0.0-1.0 × 10 before blend)
...
- **Auth**: JWT bearer tokens (HS256). Open registration at `POST /auth/register`. Login at `POST /auth/login`. `GET /auth/me` returns the user + their profiles. Password reset: `POST /auth/forgot-password` (logs link to console) → `POST /auth/reset-password` (token + new password). Access tokens (JWT) expire after `JWT_EXPIRY_DAYS` (default 7 days); password-reset tokens are separate — they expire after 30 minutes and are single-use, stored in the `password_reset_tokens` SQLite table.
